...
export class PriceConfigurationComponent {
  ...
  private buildLabelCommand(item: PriceConfigItem, preview: PriceConfigPreview): string {
    ...
    return [
      ...
      ...(preview.labelTemplate.unitLine2 ? [`^FO135,258^A0N,20,18^CI13^FR^FD${this.escapeForZpl(preview.labelTemplate.unitLine2)}^FS`] : []),
      `^FO360,${preview.labelTemplate.unitLine2 ? 278 : 240}^A0N,20,18^CI13^FR^FD${this.escapeForZpl(preview.labelTemplate.website)}^FS`,
      `^FO420,${preview.labelTemplate.unitLine2 ? 298 : 260}^A0N,20,18^CI13^FR^FD${this.escapeForZpl(preview.labelTemplate.email)}^FS`,
